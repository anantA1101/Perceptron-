...
class perceptron():
    def __init__(self, Iter, Data1 , Data2):
        """ 
        This is initializer method which loads the data on which the model has to be Trained.
        
        Iter: No of Iterations
        Data1: First class of the Data
        Data2: Second CLass of Data
         """ 
        ...
        Dataset= np.append(self.data1 , self.data2, axis=0)
    ...
    def test(self, TestData1, TestData2 ,weights, bias):
        """
        This function is used to test the model (Calculated weights and biases)
        
        TestData1: First Data To test 
        TestData2: Second Data to test 
        ...
        #Creating a combined dataset and shuffling them for better training.
        Test_Data= np.append(TestData1, TestData2, axis=0)
        print("Test Data Set : ", Test_Data)
        ...
        for i in range(len(Test_Data)):
            ...
            # If a > 0 and the Test Data classified class is equal to actual class -> Right Prediction    
                ...
                if  Test_Data[i,[4]][0]== TestData2[1,[4]][0]:
                    print("Activation Score = ", round(a[0],2),"Classified as ",TestData2[1,[4]][0] ,"correct classification")
                    right+=1
                else:
                    pass
                    print("Activation Score = ", round(a[0],2),"Classified as ",TestData2[1,[4]][0] ,"Wrong classification")

        # printing Accuracy  and Plotting 
        print("\n Out of ",len(Test_Data),right, " classifications are right ")
        print("Accuracy = ", right*100/(len(Test_Data)),"%")
...
class L2_Regularization():
    def __init__(self, Iter, All_data , Data_1, Reg_coef):
        """ 
        This is initializer method which loads the data on which the model has to be Trained.
        ...
        for i in range(self.iter):
            for j in range(len(self.X_Train)):
                ...
                if self.Y_Train[j]*a <= 0:
                    ...
                    self.weights = (1-2*self.Q)*(self.weights + self.Y_Train[j]*self.X_Train[j]) # New Updated Weights  
...
#################################################################################################
#                               Running Code (Perceptron)
#################################################################################################

""" p1= perceptron(20,Train_class_2, Train_class_3)
weights , bias = p1.train() 
print("w=",weights,"b= " ,bias)
p1.test(Test_class_2,Test_class_3,weights,bias) """

#################################################################################################
#                        Running Code (Multiclasss Perceptron)
#################################################################################################
...
#################################################################################################
#                               Running Code (L2 Perceptron )
#################################################################################################

""" p1= L2_Regularization(20,Train_dataset, Train_class_2, Reg_coef=0.01)
weights , bias = p1.train() 
print(weights, bias)
p1.test(Test_dataset, Test_class_2,weights,bias) """
